...
<h2>Estimating autocorrelation with dichotomized Gaussians</h2>

Recall that the aim is to estimate the network time constant for covariates of interest, e.g., <b>τ</b> in the right vs left hemisphere of C57 (wild-type) mice, or <b>τ</b> in the right hemisphere of C57 vs Shank3 KO mice. At first glance, the output of [process.autocorr](../reference/process.autocorr.html) appears to provide all that's needed for this statistical analysis. Why not simply take the <b>τ</b> estimates listed in the output of [process.autocorr](../reference/process.autocorr.html) and run an ANOVA? 

The problem is that recurrence is very noisy, the amount of data available from which to extract a signal through all that noise is relatively low, and <b>τ</b> itself is an imperfect measure of the network time constant. To see the problem, consider the lone cluster from a C57 mouse. Obviously a single data point is not enough to estimate the true value of a variable, but even this single data point is a highly unreliable estimate of the true network time constant of the cell. Consider what happens when slightly different initial conditions are used in the fit of the exponential decay model. First, set the initial amplitude to 0.001 (the default value in neurons): 
...
Now, set the initial amplitude to 0.01:
